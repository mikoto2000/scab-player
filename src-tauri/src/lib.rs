...
use std::{
    env, fs::create_dir_all, io::{Read, Seek, SeekFrom, Write}, sync::{Arc, Mutex}
};
...
pub fn run() {
    tauri::Builder::default()
        .setup(|app| {
            ...
            let mut conn = sqlite3::establish_connection(db_path.to_str().unwrap().to_string());
...
fn get_stream_response(
    ...
    let mut resp = Builder::new().header(CONTENT_TYPE, "audio/mp3");
    ...
    let http_response = if let Some(range_header) = request.headers().get("range") {
        ...
        if ranges.len() == 1 {
            ...
            for (end, start) in ranges {
                ...
                buf.write_all(format!("{CONTENT_TYPE}: audio/mp3\r\n").as_bytes())?;
                buf.write_all(
